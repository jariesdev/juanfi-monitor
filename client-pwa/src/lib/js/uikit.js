...
(function (global, factory) {
	...
	function isTag(element, ...tagNames) {
		return tagNames.some((tagName) => {
			var _a;
			return (
				((_a = element == null ? void 0 : element.tagName) == null ? void 0 : _a.toLowerCase()) ===
				tagName.toLowerCase()
			);
		});
	}
	...
	function boxModelAdjust(element, prop, sizing = 'border-box') {
		return css(element, 'boxSizing') === sizing
			? sumBy(
					...
					(prop2) =>
						toFloat(css(element, `padding${prop2}`)) + toFloat(css(element, `border${prop2}Width`))
			  )
			: 0;
	}
		...
		return (el[stateKey] = new Promise((resolve) => {
			...
			once(window, 'message', resolve, false, ({ data }) => {
				try {
					...
					return (
						(youtube && (data == null ? void 0 : data.id) === id && data.event === 'onReady') ||
			...
			(el) =>
				...
				((hasPosition(el, 'fixed') &&
					zIndex(
						parents(target)
							.reverse()
							.find((parent2) => !parent2.contains(el) && !hasPosition(parent2, 'static'))
					) < zIndex(el)) ||
	...
	function toList(value) {
		return isArray(value)
			...
			? value
					.split(listRe)
					.map((value2) => (isNumeric(value2) ? toNumber(value2) : toBoolean(value2.trim())))
			: [value];
	}
			...
			await Promise.all(
				getTransitionNodes(target).map(
					(child, i) =>
						new Promise((resolve) =>
							setTimeout(
								() => Transition.start(child, propsOut, duration / 2, 'ease').then(resolve),
								i * stagger
							)
						)
				)
				...
				for (const el of this.navItems) {
					...
					if (isNumeric(cmd)) {
						...
						toggleClass(
							...
							'uk-invisible',
							this.finite &&
								((cmd === 'previous' && index === 0) || (cmd === 'next' && index >= this.maxIndex))
						);
					}
				}
			}
			...
			transformFn2 = (stop) => {
				...
				return getUnit([stop])
					? toPx(stop, 'width', el, true) /
							el[
								`offset${
									((_a = stop.endsWith) == null ? void 0 : _a.call(stop, 'vh')) ? 'Height' : 'Width'
								}`
							]
					: toFloat(stop);
			};
	...
	var slider = {
				...
				for (let i = 0; i < this.length; i++) {
					...
					if (this.center) {
						if (
							...
							left +
								slideWidth +
								dimensions$1(this.slides[getIndex(+i + 1, this.slides)]).width / 2 >
								width / 2
						) {
							sets.push(+i);
		...
		update: {
			write() {
				for (const el of this.navItems) {
					...
					if (index !== false) {
						el.hidden =
							!this.maxIndex || index > this.maxIndex || (this.sets && !includes(this.sets, index));
					}
				}
				this.reorder();
		...
		events: {
			beforeitemshow(e) {
				...
				this.duration =
					speedUp(avgWidth / this.velocity) * (dimensions$1(this.slides[index]).width / avgWidth);
				this.reorder();
			},
			itemshow() {
				if (~this.prevIndex) {
		...
		methods: {
			...
			async _show(title, id) {
				...
				on(this.tooltip, 'toggled', (e, toggled) => {
					...
					this.origin =
						this.axis === 'y' ? `${flipPosition(dir)}-${align}` : `${align}-${flipPosition(dir)}`;
					const handlers = [
						once(
							document,
	...
	var Accordion = {
		...
		watch: {
			items(items, prev) {
				...
				const active =
					(this.active !== false && items[Number(this.active)]) || (!this.collapsible && items[0]);
				if (active) {
		...
		events: [
			...
			},
			{
				...
				handler({ target }) {
					if (this.$el !== target) {
						active =
							active === null && this.$el.contains(target) && this.isToggled() ? this : active;
						return;
					}
							...
							[prop]:
								(targetOffset[start] > elOffset[start]
									? targetOffset[this.inset ? end : start] -
									  Math.max(offset(this.boundary[i])[start], viewports[i][start] + viewportOffset)
									: Math.min(offset(this.boundary[i])[end], viewports[i][end] - viewportOffset) -
		...
		watch: {
			items() {
				...
				for (const container of containers) {
					const items = justify
						? $$('.uk-navbar-nav > li > a, .uk-navbar-item, .uk-navbar-toggle', container).length
						: '';
		...
		update: [
			{
				read({ height: height$1, width, margin, sticky }) {
					...
					const start =
						(this.start === false ? topOffset : parseProp(this.start, this.$el, topOffset)) -
						offset$1;
					const end =
						this.end === false
							? maxScrollHeight
			...
			},
			{
				read({
					...
					return {
						...
						below:
							scroll2 >
							offset(referenceElement).top + (sticky ? Math.min(height, elHeight) : height),
				...
				handler(e) {
					...
					let i =
						keyCode === keyMap.HOME
							...
							: keyCode === keyMap.END
							? 'last'
							: (keyCode === keyMap.LEFT && !isVertical) || (keyCode === keyMap.UP && isVertical)
							? 'previous'
							: (keyCode === keyMap.RIGHT && !isVertical) || (keyCode === keyMap.DOWN && isVertical)
							? 'next'
							: -1;
					if (~i) {
						e.preventDefault();
				...
				handler(e) {
					...
					if (
						...
						((link = e.target.closest('a[href]')) &&
							(!this.isToggled(this.target) || (link.hash && matches(this.target, link.hash))))
					) {
						e.preventDefault();
					}
